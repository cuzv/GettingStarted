...
#import "VHttpConfigure.h"
#import "Extension.h"

NSString *CHHTTPRequestMethodName = @"";

@interface VHTTPSessionManager : AFHTTPSessionManager <NSSecureCoding, NSCopying, NSMutableCopying>
...
@implementation VHTTPSessionManager

static VHTTPSessionManager *sharedInstance;
...
+ (instancetype)sharedInstance {
    ...
    dispatch_once(&onceToken, ^{
        ...
        sharedInstance = [[VHTTPSessionManager alloc] initWithBaseURL:baseURL sessionConfiguration:sessionConfiguration];
...
//        sharedInstance.requestSerializer = [AFJSONRequestSerializer serializer];
        ...
        [VHTTPSessionManager processNetworkMonitoring];
    });
...
+ (BOOL)isNetworkReachable {
    return ![VHTTPSessionManager sharedInstance].operationQueue.isSuspended;
}
...
+ (void)setHTTPRequestSerializerType:(HTTPRequestSerializerType)requestSerializerType {
    [VHTTPSessionManager sharedInstance].requestSerializerType = requestSerializerType;
}

// getting http request serializer type
+ (HTTPRequestSerializerType)requestSerializerType {
    return [VHTTPSessionManager sharedInstance].requestSerializerType;
}
...
+ (void)removeRequestByMthodName:(NSString *)methodName {
    ...
    NSURL *wantRequestURL = [[VHTTPSessionManager sharedInstance].baseURL URLByAppendingPathComponent:methodName];
    for (NSURLSessionTask *task in [VHTTPSessionManager sharedInstance].tasks) {
...
+ (void)removeAllRequest {
    [[VHTTPSessionManager sharedInstance].tasks enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        [obj cancel];
    }];
}
...
+ (void)POSTWithMethodName:(NSString *)methodName
    ...
    if (![VHTTPManager shouldContinue]) {
        return;
    }
    // remove same request
    ...
    [[VHTTPSessionManager sharedInstance] POST:methodName
